...
namespace OpenVinoSharp
{
    ...
    public class Core : IDisposable
    {
        ...
        public Model read_model(string model_path, Tensor weights)
        {
            ...
            IntPtr model_ptr = new IntPtr();
            HandleException.handler(
                NativeMethods.ov_core_read_model_from_memory_buffer(m_ptr, ref data[0], (ulong)data.Length, weights.Ptr, ref model_ptr));
            return new Model(model_ptr);
        }
        /// <summary>
        /// Reads models from IR / ONNX / PDPD / TF / TFLite formats.
        ...
        /// support model string containing several null chars.
        /// You can obtain input content through the Ov.content_from_file() method.</param>
        /// <param name="weights">Shared pointer to a constant tensor with weights.</param>
        /// <returns></returns>
        public Model read_model(byte[] xml_model_file_byte, Tensor weights) 
        {
            IntPtr model_ptr = new IntPtr();
            HandleException.handler(
                NativeMethods.ov_core_read_model_from_memory_buffer(m_ptr, ref xml_model_file_byte[0], (ulong)xml_model_file_byte.Length, weights.Ptr, ref model_ptr));
            return new Model(model_ptr);
        }
